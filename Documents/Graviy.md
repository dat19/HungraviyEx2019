...
ブラックホールの制御ができたら、ぐらびぃの移動を実装できます。ブラックホールの時と同様に、まずは状態の洗い出しから行います。
...
- 食べるアニメーション
  - 操作は通常通り
  - 吸い込み中のアイテムがある場合は、口を空けておく
  - 吸い込みが完了したら、アニメを逆再生して口を閉じる
...
これもシンプルで、排他的な動作がないので、状態を通し番号で管理する変数はなしでよいでしょう。
...
- Inhaleはどの状態からでも発動して、スクリプトのタイミングで終了する。Bool型のInhaleで制御する。また、口を閉じるアニメはSpeedを-1にしたいので、Float型のEatSpeedを定義して、SpeedのMultiplierに設定する
...
# ゲーム開始まで待機
動作が可能になるまでは待機アニメのままで静止させておきます。移動処理は物理演算ベースで行います。
...
- Rigidbody2DのAddForceで、距離に応じた力を加える
- 力は更新間隔に関係なく設定したいので、ForceMode2D.Forceを使う
